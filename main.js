...
alienSprite = [[new Sprite(this, 0, 0, 22, 16), new Sprite(this, 0, 16, 22, 16)],      
[new Sprite(this, 22, 0, 16, 16), new Sprite(this, 22, 16, 22, 16)],  
[new Sprite(this, 38, 0, 24, 16), new Sprite(this, 38, 16, 24, 16)],    
      // Create tank object
      tankSprite = newSprite(this, 62, 0, 22, 16);
tank = {
...
citySprite = new Sprite(this, 84, 8, 24, 16);
...
for(let i = 0; i < rows.length; i++){
  ...
  */
  ...
  function update(){
    ...
    // Moving the tank
    if(keyPressed.indexOf(65) != -1){
tank.x -= 4;
}
if(keyPressed.index(68)!= -1){
  tank.x += 4;
}
    // Restricting the tank's position to within the screen
    tank.x = Math.max(30, tank.x);
tank.x = Math.min(screen.width -tank.width - 30, tank.x);
    ...
    for(let i = 0; i < bullets.length; i++){
      ...
      if(cites.y < bullet.y +bullet.height.&&.
    ...
    if(Math.random() < 0.03 && aliens.length > 0){
      ...
      bullets.push(new Bullet(alien.x + alien.width = 0.5, alien.y + alien.height, 4, 2, 4, "#FFFFFFF"))
    }
...
screen.clear()    
    ...
    for(let i=0; i < aliens.length; i++){
      let alien = aliens[i];
      screen.drawSprite(alienSprite[motion], alien.x, alien.y);
    }
